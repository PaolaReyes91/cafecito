...
export default function POS() {
  ...
  // --- LÓGICA DE CÁLCULO DE TOTALES Y DESCUENTOS ---
  ...
  const getDiscountPercentage = (purchasesCount) => {
    ...
    if (purchasesCount >= 4 && purchasesCount <= 7) return 0.10; // Corregido: 0.10 (10%) en lugar de 10
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        total: finalTotal // Enviamos el total real con el descuento aplicado al backend
      };
              ...
              <div className="cart-summary-totals">
                ...
                {customer && (
                  <div className="summary-row discount-text">
                    <span className="descuento-texto">Descuento de Fidelización</span>
                    <span className="descuento-monto">-${discountAmount.toFixed(2)}</span>
                  </div>
                )}

                <div className="cart-total">
                  <span>Total</span>
                  {/* Corregido: Ahora renderiza la variable local finalTotal */}
