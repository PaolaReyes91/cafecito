...
export default function CustomerList() {
  ...
  const handleIdSearch = async () => {
    ...
    setIdResult(null);
    try {
      const res = await getCustomerById(idSearch.trim());
      setIdResult(res);
    } catch (err) {
      if (err.response?.status === 404) {
        setIdError('Cliente no encontrado');
      } else {
        setIdError('Error al buscar cliente');
      }
    } finally {
      setIdLoading(false);
    }
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setCreateError(err.response?.data?.error || 'Error al crear cliente');
    } finally {
  ...
  return (
    <div className="customer-list">
      ...
      <div className="id-search">
        ...
        />
        <button onClick={handleIdSearch} disabled={idLoading}>
          {idLoading ? 'Buscando...' : 'Buscar'}
        </button>
      </div>
      {idResult && (
        <div className="id-result">
          <button className="close-btn" onClick={() => setIdResult(null)}>×</button>
          <p><strong>{idResult.name}</strong></p>
              <p>{idResult.phoneOrEmail}</p>
              <p>Compras: {idResult.purchasesCount}</p>
        </div>
      )}
