...
export default function ProductList({ viewOnly = false }) {
  ...
  return (
    <div className="product-list">
      <div className="list-header">
        <h2>{viewOnly ? 'Productos' : 'Productos'}</h2>
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <th>Precio</th>
                <th>Stock</th>
                {!viewOnly && <th>Acciones</th>}
              </tr>
            </thead>
            <tbody>
              {products.length === 0 ? (
                <tr><td colSpan={viewOnly ? 3 : 4} className="empty">No hay productos</td></tr>
              ) : products.map((p) => (
                <tr key={p.id}>
                  <td>{p.name}</td>
